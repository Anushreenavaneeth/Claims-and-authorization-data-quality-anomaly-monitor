...
SLA Score formula (record level):
    ...
    where weights come from config["record_scoring_weights"].

This module wraps the logic already proven in `sla risk/sla monitoring.py`
and adapts it to work with StandardAnomalyRecord objects.
"""
...
def _default_config() -> Dict[str, Any]:
    return {
        ...
        "risk_level_thresholds":  {"LOW": {"min": 0, "max": 30}, "MEDIUM": {"min": 30, "max": 60}, "HIGH": {"min": 60, "max": 80}, "CRITICAL": {"min": 80, "max": 100}},
...
def apply_sla(record: StandardAnomalyRecord) -> StandardAnomalyRecord:
    """
    Calculate and attach SLA information to a StandardAnomalyRecord.
...
def _calculate_sla(record: StandardAnomalyRecord) -> SLABlock:
    ...
    # Anomaly contribution is 100 if anomalous
    anomaly_contribution = 100.0
