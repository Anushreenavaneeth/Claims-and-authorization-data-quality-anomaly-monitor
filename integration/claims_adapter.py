...
Claims Adapter
==============
Converts one raw claims JSON record (tc_puf_final_anomaly_results.json format)
into a StandardAnomalyRecord.

Input fields used:
  record_id.plan_id / record_id.issuer_id
  entity.*
  final_assessment.*
  bayesian.*
  rule_engine.*
  ml_evidence.*
  context_for_rag (optional)

Does NOT re-run any ML logic — purely normalises the existing output.
"""
...
def adapt(raw: Dict[str, Any]) -> StandardAnomalyRecord:
    ...
    try:
        ...
        issuer_id = str(rid.get("issuer_id", "") or "")
        raw_key   = plan_id or issuer_id or "UNKNOWN"
        record.record_id = make_record_id("CLAIMS", raw_key)
        ...
        if record.anomaly.is_anomaly:
            issues.append(f"Anomaly detected with {severity} severity")
        ...
        ml_score   = _derive_anomaly_score(raw)
        ml_reasons = []
        if ml_ev.get("summary"):
            ml_reasons.append(str(ml_ev["summary"]))
        if ml_ev.get("details"):
            ml_reasons.append(str(ml_ev["details"]))
        ...
        rule_names     = [r.strip() for r in rule_names_raw.split(";") if r.strip() and r.strip() != "NONE"]
        violations_raw = str(rule_eng.get("reason", "") or "")
        violations     = [v.strip() for v in violations_raw.split(";") if v.strip() and v.strip().lower() != "no rule violation detected"]
        ...
        record.rules = RulesBlock(
            ...
            severity        = rule_severity if rule_severity != "NONE" else "NONE",
        )
        ...
        if record.bayesian.is_anomaly:
            evidence.append(f"Bayesian probability {bay_prob:.4f} exceeds threshold")
        if ml_ev.get("summary"):
            evidence.append(str(ml_ev["summary"]))
        if raw.get("context_for_rag"):
            evidence.append(str(raw["context_for_rag"]))
        record.evidence = evidence

        record.processing_status = "adapted"

    except Exception as exc:
        logger.exception("ClaimsAdapter error for record: %s", raw)
        record.processing_errors.append(f"ClaimsAdapter: {exc}")
...
def _derive_anomaly_score(raw: Dict[str, Any]) -> float:
    """
    Derive a 0-1 normalised anomaly score.
    Claims ML evidence may not always carry an explicit score;
    we fall back to Bayesian probability when ML evidence is empty.
    """
    ml_ev = raw.get("ml_evidence", {})
    evidence_count = int(ml_ev.get("evidence_count", 0) or 0)

    if evidence_count > 0:
        # Heuristic: more evidence → higher score
        raw_score = min(evidence_count / 5.0, 1.0)
        return round(raw_score, 4)

    # Fall back to Bayesian probability normalised to [0,1]
    bay = raw.get("bayesian", {})
    prob = float(bay.get("probability", 0) or 0)

    # Claims Bayesian probabilities are already in [0,1]
    if 0.0 <= prob <= 1.0:
        return round(prob, 4)

    # Some records store score (not probability); normalise against threshold
    score = float(bay.get("score", 0) or 0)
    threshold = float(bay.get("threshold", 1) or 1)
    if threshold > 0:
        return round(min(score / threshold, 1.0), 4)
